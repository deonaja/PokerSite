...
- [ ] **Visual redesign pass — "Underground Table"** (started 2026-05-30, owner-directed): bahasa visual baru di atas primitive shadcn, tetap felt-green/dark/anti-AI-ish.
  - [x] Dashboard: player list jadi **standings table** (rank by saldo, kolom mono, divider — owner nolak versi accent-bar felt kiri); low-balance gold (⚠); season info jadi "strip" lega (bukan card padat); CTA chunky uppercase. `PlayerCard` dihapus (digantikan standings inline di DashboardClient).
