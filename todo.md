...
- [ ] **Visual redesign pass — "Underground Table"** (started 2026-05-30, owner-directed): bahasa visual baru di atas primitive shadcn, tetap felt-green/dark/anti-AI-ish.
  - [x] Dashboard: **redesign total ke konsep hero "SALDO KAMU"** (owner-approved setelah beberapa iterasi). Hero = saldo pemain login (angka mono gede) + delta musim (▲/▼ warna) + 2 baris meta (peringkat #N dari M; Season N · phase). Di bawahnya **KLASEMEN** = standings flat (rank by saldo, kolom flex sejajar, low-balance ⚠ gold), baris pemain-login di-highlight (bg felt tipis + tag KAMU). CTA chunky. `page.tsx` passing `currentPlayerId` via `getAuthenticatedPlayer()`. `PlayerCard` dihapus. (Iterasi yang ditolak: accent-bar felt kiri, chip-pill, box container, garis pembatas, season band felt — semua direvert.)
  ...
  - [ ] Roll out bahasa visual ke layar lain: session aktif (dealer = chip token ●, accent-bar peserta), setup, end-wizard, admin, season/new, history, player, pin, identity-picker.
