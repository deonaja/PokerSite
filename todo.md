...
- [ ] **Visual redesign pass — "Underground Table"** (started 2026-05-30, owner-directed): bahasa visual baru di atas primitive shadcn, tetap felt-green/dark/anti-AI-ish.
  ...
  - [ ] Roll out bahasa visual ke layar lain (1-per-1, render PNG dulu utk review):
    ...
    - [ ] admin — DITUNDA (utility di balik key, udah themed re-platform; bisa di-vibe-in nanti kalau owner mau).
- [ ] **Known infra: `global-teardown.ts:63` error** — cleanup teardown kadang lempar error (kemungkinan FK saat hapus `[T…]` players yang ke-refer `season_results`/sessions), bikin `pnpm test` exit non-zero walau semua test pass. Pre-existing (muncul di HEAD bersih). Belum diinvestigasi tuntas — test hasilnya tetap valid.
