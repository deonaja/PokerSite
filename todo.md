...
- **Revisi (2026-08-24, owner):** numeric teletext page codes (P100/P200/…) replaced with **block-mosaic pixel icons** (`components/PixelIcon.tsx`, 5×5 SVG glyphs) on every header + dashboard tabs — codes were non-functional decoration on touch. Dashboard tabs now **Saldo · Profil · Riwayat** (SESI dropped; PROFIL → `/player/[id]`). Commit `536b1ed`. `pnpm build` clean, typecheck green, verified in real app.
- **PROD OUTAGE FIXED (2026-08-24):** pokeraja.vercel.app 500'd on `/api/poll`. Root cause: Vercel prod DB is a SEPARATE Neon endpoint (`ep-proud-sound-ao8lys8z`) from dev/poker-online (`ep-bold-glitter-aoqrs5yq`), and was missing the `sessions.mode` column (online_001 migration never applied there). Applied `online_001_session_mode.sql` (mode col + indexes) directly to the prod DB; poll now 200. Prod DB has real data (10 players, 5 seasons, 86 sessions). **Owner pasted the prod DB owner password in chat → rotate it in Neon + update Vercel env.** The two DBs are separate; unifying is optional future cleanup.
- **DONE this session**: full teletext redesign (all screens) + icon revision + create-season deadlock fix + prod DB fix. `pnpm build` clean. Pushed to dev / PR #6 to main.
