...
export interface ParticipantTreatment {
  ...
  salaryChips: boolean
  /** Playing free dealer additionally gets 1× buy_in credited to bankroll
   *  (the 2× split spare life). Neutral dealers never get this. */
...
export function deriveParticipantTreatment(opts: {
  ...
  if (!dealerPlays) {
    if (dealerFreeEntry) {
      ...
      return { deduction: 0, action: 'buy_in_dealer_free', noGaji: true, salaryChips: true, salaryBankroll: false }
    }
  ...
  // PLAYING dealer.
  if (dealerFreeEntry) {
    // Phase 1, not cooled down → free entry + 2× split (table + bankroll).
    return { deduction: 0, action: 'buy_in_dealer_free', noGaji: false, salaryChips: true, salaryBankroll: true }
  }
  if (balance < buyIn) {
